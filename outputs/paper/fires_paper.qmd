...
The Toronto Fire Service (TFS) responds to thousands of fire events each year. TFS is the largest fire service in Canada and the fifth-largest in [North America](https://www.toronto.ca/city-government/accountability-operations-customer-service/city-administration/staff-directory-divisions-and-customer-service/fire-services/) [@TFSCite], with 85 fire stations that serve a population of nearly 3 million people in the City of Toronto. TFS keeps records of all of the incidents for which they were called. TFS and the Ontario Fire Marshall provide data about incidents in the city about the source, effects and response times to fires and post the information on OpenData Toronto [@FireDataCite]. 
...
Ward 13 saw the most civilians deaths from fires, while Ward 23, Scarborough North, had the fewest fire-related civilian casualties (@fig-4).
...
merged_ward_fires |>
 ...
 # code to show labels on the plot points referenced from https://github.com/InessaDeAngelis/Toronto_Elections/blob/main/outputs/paper/toronto_elections_paper.qmd 
  geom_text(aes(label = ward), vjust = -0.8, hjust = 0.8) +  
