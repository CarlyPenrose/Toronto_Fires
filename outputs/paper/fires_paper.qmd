...
The Toronto Fire Service (TFS) responds to thousands of fire events each year. TFS is the largest fire service in Canada and the fifth-largest in [North America](https://www.toronto.ca/city-government/accountability-operations-customer-service/city-administration/staff-directory-divisions-and-customer-service/fire-services/) [@TFSCite], with 85 fire stations that serve a population of nearly 3 million people across 25 wards. TFS keeps records of all of the incidents for which they were called and reports them to the Ontario Fire Marshall. The Ontario Fire Marshall provides this data to the city, which is then published on OpenData Toronto [@FireDataCite]. 
...
This paper tests the hypothesis that lower income areas have more dire outcomes from fires. First, it explores the number of fire incidents across years and across wards. Next, it looks at where civilian casualties from TFS-reported fire incidents were greatest from years 2018 to 2022. Finally, it plots the median household income in the ward according to 2021 census data against the number of civilians casualties to understand the relationship between median income and deaths from fires [@WardDataCite].
...
The columns chosen for this analysis were; number of civilian casualties, TFS alarm time (used to identify the year of the incident) and incident ward. TFS alarm time was used to isolate the "year" variable because that data was more complete than the "incident time" data. Soem potentially relevant columns were excluded. These include civilian injuries from fire incidents and the cost of damage from the fire. TFS firefighter casualties were not included in this analysis because, according to a 2014 [report](http://nfidcanada.ca/wp-content/uploads/2017/09/Fire-statistics-in-Canada-2005-to-2014.pdf) civilian casualties represent as many as 99% of deaths from structure fires, so the inclusion of firefighter deaths would be unlikely to change the analysis. [@NFIDstudy]. 
...
There appears to be a negative relationship between median income in a ward and the number of civilian casualties from fires in Toronto. The higher the median income of a ward, the fewer civilian deaths occurr in that ward (@fig-5).
